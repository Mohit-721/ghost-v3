...
Ghost v3 consists of three core Agent layers:
1. **Agent 1 (Infrastructure)**: SQlite memory handlers, EventBus pub/sub, Concurrent queue writer, and structural Data objects.
2. **Agent 2 (Synthesis)**: Cost aggregators, Context assemblers via RAG, OpenAI interactions, intent routers, and the Tool quarantine Forge.
3. **Agent 3 (Interface)**: The UDS running Uvicorn daemon process mapped to FastAPI syncing natively to the Typer-based synchronizer CLI.
